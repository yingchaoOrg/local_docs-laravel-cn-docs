...
如果你从 2.x 发布系列升级到 Sanctum 3.x，请参考 [Sanctum 升级指南](https://github.com/laravel/sanctum/blob/3.x/UPGRADE.md)。
...
Redis 的 [缓存标签](/docs/laravel/10.x/cache#cache-tags) 支持已被重新编写，以提供更好的性能和存储效率。在 Laravel 之前的版本中，如果使用 Redis 作为应用程序的缓存驱动程序，则会在缓存中累积旧的缓存标签。
